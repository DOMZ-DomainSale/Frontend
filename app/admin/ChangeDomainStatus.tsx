...
type Props = {
  status: string // "Pass" | "Fail" | "Manual Review"
  domainId: string
...
export default function ChangeDomainStatus({
  ...
  const submitChange = async (nextStatus: DomainStatus) => {
    if (loading) return
  ...
  return (
    <div className="relative inline-block">
      ...
      {open && (
        <div className="absolute z-20 mt-1 w-36 rounded-md border bg-white shadow-md">
          {safeStatus !== "pass" && (
            <button
              onClick={() => submitChange("pass")}
